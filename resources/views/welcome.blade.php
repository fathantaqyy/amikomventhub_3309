...
@section('content')
    ...
    <section class="max-w-7xl mx-auto px-6 py-20">
        <div class="text-center mb-14">
            <h2 class="text-4xl font-extrabold mb-4">Partner AmikomEventHub</h2>
            <p class="text-slate-500 text-lg mb-8">Platform ini didukung oleh berbagai partner terbaik</p>

            <form action="{{ url('/') }}" method="GET" class="max-w-md mx-auto flex gap-2">
                <input type="text" name="search" placeholder="Cari partner..." value="{{ request('search') }}" class="flex-1 px-6 py-3 rounded-xl border-slate-200 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm border">
            ...
            @forelse($partners as $partner)
                <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 hover:shadow-xl transition text-center group">
                    <div class="w-24 h-24 mx-auto mb-4 flex items-center justify-center">
                        <img src="{{ $partner->logo_url }}" alt="{{ $partner->name }}" class="max-w-full max-h-full object-contain group-hover:scale-110 transition-transform" onerror="this.onerror=null;this.src='https://ui-avatars.com/api/?name={{ urlencode($partner->name) }}&background=6366f1&color=fff';">
                    </div>
